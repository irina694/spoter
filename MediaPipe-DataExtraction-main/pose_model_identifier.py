...
class mp_holistic_data:
    def __init__(self, video_id, gloss_id, column_names):
        self.data_hub = {}
        for n in column_names[1:-1]:
            self.data_hub[n] = []
        self.data_hub['video_id'] = video_id
        self.data_hub['labels'] = gloss_id
	...
	# for the frame that can not extract skeleton from
        ...
        for handedness, lm in zip(['Right','Left'],[holistic_results.right_hand_landmarks,holistic_results.left_hand_landmarks] ):
            if lm:
